...
homekit_characteristic_t revision = HOMEKIT_CHARACTERISTIC_(FIRMWARE_REVISION, "0.0.7");


// GENERAL AND CUSTOM


homekit_characteristic_t temp_offset = HOMEKIT_CHARACTERISTIC_(CUSTOM_TEMPERATURE_OFFSET, 0.0,
    .id = 108, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(change_settings_callback));


homekit_characteristic_t ip_addr
    = HOMEKIT_CHARACTERISTIC_(CUSTOM_IP_ADDR, "", .id = 109, .getter = read_ip_addr);


homekit_characteristic_t reboot_device = HOMEKIT_CHARACTERISTIC_(CUSTOM_REBOOT_DEVICE, false,
    .id = 110, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(reboot_callback));


homekit_characteristic_t ota_firmware = HOMEKIT_CHARACTERISTIC_(CUSTOM_OTA_UPDATE, false, .id = 111,
    .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(ota_firmware_callback));
...
homekit_characteristic_t current_heating_cooling_state
    = HOMEKIT_CHARACTERISTIC_(CURRENT_HEATING_COOLING_STATE, 0);
...
// homekit_characteristic_t cooling_threshold =
// HOMEKIT_CHARACTERISTIC_(COOLING_THRESHOLD_TEMPERATURE, 22,
// .callback=HOMEKIT_CHARACTERISTIC_CALLBACK(on_temp_update),.min_value = (float[]) {18},.max_value
// = (float[]) {30},.min_step = (float[]) {1} );


// homekit_characteristic_t heating_threshold =
// HOMEKIT_CHARACTERISTIC_(HEATING_THRESHOLD_TEMPERATURE, 28,
// .callback=HOMEKIT_CHARACTERISTIC_CALLBACK(on_temp_update),.min_value = (float[]) {16},.max_value
// = (float[]) {30},.min_step = (float[]) {1});
...
void update_state()
{
    ...
    homekit_characteristic_notify(
        &current_heating_cooling_state, current_heating_cooling_state.value);
...
void device_restart()
{
    ...
    xTaskCreate(
        device_restart_task, "device_restart_task", configMINIMAL_STACK_SIZE, NULL, 1, NULL);
}
