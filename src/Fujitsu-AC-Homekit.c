...
uint8_t state = target_heating_cooling_state.value.int_value;
        ...
        if (state == 1 && current_temperature.value.int_value > target_heating_cooling_state.value.int_value) {
	current_heating_cooling_state.value = HOMEKIT_UINT8(0);
        }else if (state == 2 && current_temperature.value.int_value < target_heating_cooling_state.value.int_value) {
	current_heating_cooling_state.value = HOMEKIT_UINT8(0);
        }else{
...
void update_temp() {
        ...
        //there is an if because i was using treshold method
	if (target_state == 1) {
	//Read the Heat target
	target_temp1= target_temperature.value.float_value;
	}
